import 'package:flutter/material.dart';
import 'package:flutter_bloc/flutter_bloc.dart';

import '../../../../controllers/audio_controller/audio_cubit.dart';
import '../../../../core/components/bloc_consumer_widget.dart';
import '../../../../core/components/custom_widgets/custom_row.dart';
import '../../../../core/components/custom_widgets/custom_text.dart';
import '../../../../core/functions/format_audio_duration.dart';
import '../../../../core/resources/font_manager.dart';

class AudioDurationWidget extends StatelessWidget {
  const AudioDurationWidget({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocConsumerWidget<AudioCubit, AudioState>(
      listener: (context, state) {},
      builder: (context, state) {
        return CustomRow(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            CustomText(
              data: formatAudioDuration(context.read<AudioCubit>().position ~/ 1000).toString(),
              style: Theme.of(context).textTheme.bodySmall!.copyWith(
                fontSize: FontManager.s08,
              ),
            ),

            CustomText(
              data: formatAudioDuration(context.read<AudioCubit>().currentAudioDuration.inSeconds).toString(),
              style: Theme.of(context).textTheme.bodySmall!.copyWith(
                fontSize: FontManager.s08,
              ),
            ),
          ],
        );
      },
    );
  }
}
